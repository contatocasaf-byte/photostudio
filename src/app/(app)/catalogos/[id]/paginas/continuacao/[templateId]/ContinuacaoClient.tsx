...
export default function ContinuacaoEditorPage({ params }: { params: Promise<{ id: string; templateId: string }> }) {
  ...
  // Reaproveitar configuração de outra variante de continuação do mesmo
  // catálogo — mesma mecânica já usada na abertura de seção
  // (handleCopyFrom): "Copiar" só carrega os valores no estado local,
  // não persiste nada nem religa as duas variantes, o usuário ainda
  // precisa clicar Salvar.
  const [otherContinuacoes, setOtherContinuacoes] = useState<ContinuacaoListItem[]>([]);
  const [copySourceId, setCopySourceId] = useState("");
  const [copying, setCopying] = useState(false);

  useEffect(() => {
    let cancelled = false;
    Promise.all([getContinuacaoTemplate(templateId), getCatalog(catalogId), listContinuacaoTemplates(catalogId)]).then(
      ([detailRes, catalogRes, othersRes]) => {
        ...
        if (!othersRes.error) setOtherContinuacoes((othersRes.templates ?? []).filter((t) => t.id !== templateId));
        setLoading(false);
      }
    );
    return () => {
  ...
  async function handleCopyFrom() {
    ...
    setStatus(null);
    try {
      const res = await getContinuacaoTemplate(copySourceId);
      if (res.error || !res.detail) {
        setStatus(`⚠ ${res.error ?? "Continuação não encontrada."}`);
        return;
      }
  ...
  async function handleSave() {
    ...
    try {
      const [sizeRes, nomeRes, templateRes] = await Promise.all([
        ...
        saveContinuacaoTemplate(templateId, { layout, elementosHabilitados, margens, fundoKey, fundoPdfKey, illustracoes, formas }),
      ]);
    ...
    <div>
      ...
      {otherContinuacoes.length > 0 && (
        <div className="mt-3 flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
          ...
          >
            <option value="">Selecione uma continuação...</option>
            {otherContinuacoes.map((t) => (
