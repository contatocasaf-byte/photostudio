...
export type StudioAcessos = Record<Tab, boolean>;

// Conteúdo original de studio/page.tsx (Fase 6: virou Client Component
// separado, recebendo de page.tsx — agora Server Component que checa
// permissão — quais das 4 abas esta conta pode ver. Só a aba
// selecionada some da UI; a permissão de verdade continua sendo
// aplicada no gate da própria page.tsx, não aqui.
export default function StudioClient({ acessos }: { acessos: StudioAcessos }) {
  const tabs = ALL_TABS.filter((t) => acessos[t.id]);
  const [tab, setTab] = useState<Tab>(tabs[0]?.id ?? "lote");
  ...
  return (
    <div className={tab === "renomear" ? "max-w-5xl" : "max-w-3xl"}>
      ...
      <div className="mt-4">
        {tab === "lote" && acessos.lote && <BatchUpload />}
        {tab === "marca" && acessos.marca && (
          <WatermarkTool incomingFiles={incomingMarcaFiles} onConsumedIncoming={() => setIncomingMarcaFiles(undefined)} />
        )}
        {tab === "renomear" && acessos.renomear && <RenamerTool />}
        {tab === "comparar" && acessos.comparar && (
          <FolderCompareTool
            onApplyMarca={(files) => {
