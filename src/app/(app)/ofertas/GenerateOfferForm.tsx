...
export default function GenerateOfferForm() {
  ...
  // Config do layout (posições/textos/cores) — carregada de forma
  // assíncrona: busca o .json salvo no R2 (Editor de Layout, Fase 4a)
  // mesclado por cima dos padrões calculados pro tamanho real do
  // arquivo; sem config salva, cai direto nos padrões.
  ...
  useEffect(() => {
    ...
    // onChange(null)) — layoutCfg já nasce null, não precisa resetar.
    if (!layout) return;
    let cancelled = false;
    loadLayoutConfig(layout).then((cfg) => {
      if (!cancelled) setLayoutCfg(cfg);
    });
    return () => {
      cancelled = true;
    };
