...
// redor do objeto em imagens PNG, centralizando pelo centro de MASSA
// do conteúdo sólido — não pelo centro geométrico do retângulo.
// Promovido de ofertas/core/fitImageOnCanvas.ts (mesmo raciocínio já
// aplicado a drawTextFit/loadImage): o campo "Ilustração" do editor de
// página do Criador de Catálogos também precisa da mesma lógica de
// "encaixar sem cortar o objeto visível" — nada aqui é específico de
// oferta.
//
// Diferença deliberada em relação ao app original: o original usava
// "cover fit" (Math.max entre as duas razões box/objeto) — preenche a
// caixa inteira, mas corta o que sobrar num dos eixos sempre que a
// proporção do objeto não bate com a da caixa. Aqui usa "contain fit"
// (Math.min) com uma margem de respiro (TARGET_FILL_RATIO) — o objeto
// sempre cabe inteiro dentro da caixa, ocupando ~85-90% dela, nunca
// cortado.
...
// objeto (não a sombra) define a área de referência.
...
// Redimensiona a imagem pra caber inteira dentro da caixa (boxW x
// boxH), sem distorcer a proporção (equivalente a "object-fit:
// contain") e sem NUNCA cortar o objeto visível — a escala é calculada
// sobre a bounding box do conteúdo opaco, não sobre o retângulo inteiro
// do arquivo (ignora a margem transparente ao redor), com uma margem
// de respiro (TARGET_FILL_RATIO ~87,5%) em vez de encostar exatamente
// nas bordas da caixa.
